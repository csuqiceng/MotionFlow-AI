...
export function FlowDraftEditor({ draft, commands = [], onSave, saving = false }: { draft: EngineerFlowDraft; commands?: LibraryCommand[]; onSave: (draft: EngineerFlowDraft) => void | Promise<void>; saving?: boolean }) {
  ...
  return <form className="flex flex-col gap-3 p-4" onSubmit={(event) => { event.preventDefault(); submit(); }}>
    ...
    {value.steps.map((step, index) => <fieldset key={`${step.step_id}-${index}`} className="grid gap-2 rounded border p-2"><legend>{commands.length > 0 ? `${index + 1}. ${step.action}` : t("library.workbench.step", { number: index + 1 })}</legend>{commands.length === 0 && <><Input aria-label={t("library.workbench.stepId", { number: index + 1 })} type="number" value={step.step_id} onChange={(e) => patch(index, { step_id: Number(e.target.value) })} /><Input aria-label={t("library.workbench.stepAction", { number: index + 1 })} value={step.action} onChange={(e) => patch(index, { action: e.target.value })} /><Input aria-label={t("library.workbench.stepFunction", { number: index + 1 })} type="number" value={step.func_id} onChange={(e) => patch(index, { func_id: Number(e.target.value) })} /></>}<Input aria-label={t("library.workbench.stepSpeed", { number: index + 1 })} type="number" value={step.spd_pct} onChange={(e) => patch(index, { spd_pct: Number(e.target.value) })} />{commands.length === 0 && <textarea aria-label={t("library.workbench.stepParametersJson", { number: index + 1 })} value={typeof step.params === "string" ? step.params : JSON.stringify(step.params)} onChange={(e) => patch(index, { params: e.target.value })} />}{errors[index] && <p role="alert">{errors[index]}</p>}<div><Button type="button" aria-label={t("library.workbench.moveStepUp", { number: index + 1 })} onClick={() => move(index, -1)}>{t("library.workbench.moveUp")}</Button><Button type="button" aria-label={t("library.workbench.moveStepDown", { number: index + 1 })} onClick={() => move(index, 1)}>{t("library.workbench.moveDown")}</Button><Button type="button" aria-label={t("library.workbench.removeStep", { number: index + 1 })} onClick={() => setValue((current) => ({ ...current, steps: current.steps.filter((_, i) => i !== index) }))}>{t("library.workbench.remove")}</Button></div></fieldset>)}

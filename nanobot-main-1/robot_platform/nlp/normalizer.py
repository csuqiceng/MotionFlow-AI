...
# Test and migration compatibility override. ``None`` means use the runtime
# data directory; the platform itself never reads host configuration.
_DEFAULT_CONFIG_PATH: Path | None = None

def _default_config_path() -> Path:
    """Resolve the vocabulary from the host-configured platform data directory."""
    if _DEFAULT_CONFIG_PATH is not None:
        return _DEFAULT_CONFIG_PATH
    from robot_platform.runtime import get_robot_data_dir

    return get_robot_data_dir() / "nlp_standard_words.json"

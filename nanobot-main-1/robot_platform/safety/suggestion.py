...
Ported verbatim from the legacy Qt project
(``robot_modbus_lite/safety_suggestion.py``). Pure logic — no Qt coupling.
Only adaptation: ``AxisRangeConfig`` -> ``SafetyLimits`` import.
"""
...
from robot_platform.safety.config import SafetyLimits


class SafetySuggestionService:
    """Clamps adjustable plan fields to configured L1 limits."""

    def __init__(self, config: SafetyLimits) -> None:
        self.config = config

"""Controller-independent L1 safety precheck service.

Ported from the legacy Qt project (``robot_modbus_lite/safety_precheck.py``).
Pure logic — no Qt coupling. Only adaptation: ``AxisRangeConfig`` ->
``SafetyLimits`` and the literal ``650.0`` Z-pivot -> ``Z_PIVOT_MM``.
"""
...
from robot_platform.safety.config import SafetyLimits, Z_PIVOT_MM
...
class SafetyPrecheckService:
    ...
    def run_l1(self, snapshot: dict[str, Any], plan: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        items = [
            ...
            self._item(
                "alarm", "L1", "无活动报警", not bool(safety.get("alarm_active")), "当前没有活动报警。", "当前存在活动报警。"
            ),
            self._item(
                "paused", "L1", "未处于暂停状态", not bool(safety.get("paused")), "系统未暂停。", "系统处于暂停状态。"
            ),
            self._item(
                "controller",
                "L1",
    ...
    def _target_safety_r_item(self, target: dict[str, Any]) -> dict[str, str] | None:
        ...
        sphere_r = math.sqrt(
            x_value * x_value + y_value * y_value + (z_value - Z_PIVOT_MM) * (z_value - Z_PIVOT_MM)
        )
        ...
        inner_distance = horizontal_r if z_value <= Z_PIVOT_MM else sphere_r
        if self.config.safe_r_min > 0 and inner_distance < self.config.safe_r_min:
            zone = "圆柱区" if z_value <= Z_PIVOT_MM else "半球区"
            return self._item(
                "target_r_range",
                "L1",
                "目标内径在安全范围内",

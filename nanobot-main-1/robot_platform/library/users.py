...
def _nanobot_home() -> Path:
    """Compatibility name for the host-configured platform runtime parent."""
    from robot_platform.runtime import get_robot_data_dir

    return get_robot_data_dir().parent
...
def migrate_users_if_needed(*, users_path: str | Path | None = None,
                             legacy_engineer_password_hash: str = "", b1a_config_path=None,
    ...
    admin_hash = str(legacy_engineer_password_hash or "")
    if not admin_hash and b1a_config_path is not None:
        # Read the legacy JSON shape directly.  This compatibility path is
        # deliberately data-only: the platform must not import nanobot config.
        try:
            raw_config = json.loads(Path(b1a_config_path).read_text(encoding="utf-8"))
            robot_config = raw_config.get("robotAi", raw_config.get("robot_ai", {}))
            engineer = robot_config.get("engineer", {}) if isinstance(robot_config, dict) else {}
            admin_hash = str(engineer.get("passwordHash", engineer.get("password_hash", "")) or "")
...
def initialize_user_identity(*, users_path: str | Path | None = None,
                              ...
                              legacy_engineer_password_hash: str = "",
                              b1a_config_path=None,
    ...
    migrate_users_if_needed(users_path=upath,
                             legacy_engineer_password_hash=legacy_engineer_password_hash,
                             b1a_config_path=b1a_config_path,

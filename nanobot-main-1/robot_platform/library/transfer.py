...
def _import_entities(items: list[dict[str, Any]], registry: Any, collection: str, id_key: str,
    ...
    for source in items:
        ...
        # Operator-facing projections use the published display name as a
        # lookup key.  A distinct ID with the same name would otherwise be
        # silently hidden from the command/flow pages.  Keep imports
        # lossless: skip reports the collision, while rename makes it visible.
        display_name = _entity_display_name(entity)
        if display_name and _display_name_owned_by_other(target, display_name, entity_id):
            if strategy != "rename":
                result["skipped"].append({"id": entity_id, "code": "name_conflict"})
                continue
            entity_id = _next_import_id(
                entity_id,
                target,
                separator="-" if collection == "commands" else "_",
            )
            _replace_entity_id(entity, id_key, entity_id)
...
def _entity_display_name(entity: dict[str, Any]) -> str:
    draft = entity.get("draft")
    if isinstance(draft, dict) and str(draft.get("name", "")).strip():
        return str(draft["name"]).strip()
    versions = entity.get("versions")
    published = entity.get("published_version")
    if isinstance(versions, dict) and published is not None:
        version = versions.get(str(published))
        if isinstance(version, dict) and str(version.get("name", "")).strip():
            return str(version["name"]).strip()
    return ""


def _display_name_owned_by_other(
    target: dict[str, Any], name: str, entity_id: str,
) -> bool:
    normalized = name.casefold()
    return any(
        existing_id != entity_id
        and isinstance(existing, dict)
        and _entity_display_name(existing).casefold() == normalized
        for existing_id, existing in target.items()
    )


def _next_import_name(base: str, target: dict[str, Any]) -> str:
    existing = {
        _entity_display_name(entity).casefold()
...
def _replace_entity_name(entity: dict[str, Any], name: str) -> None:
    for version in [entity.get("draft"), *(entity.get("versions", {}) or {}).values()]:
        if isinstance(version, dict):
            version["name"] = name

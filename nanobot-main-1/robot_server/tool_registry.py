"""Capability-, role- and enablement-aware registry for product Tools."""
...
class ToolEligibility:
    """Explain why a Tool may or may not be exposed to an AI runtime."""
...
class ToolRegistry:
    """Own product Tool manifests without depending on an agent SDK registry."""
    ...
    def evaluate(
        ...
        *,
        ...
        if enabled is not None and manifest.tool_id not in enabled:
...
def _supports_capability(capabilities: Mapping[str, object], capability: str) -> bool:
    """Read the public v1 capability contract without a vendor-specific branch."""

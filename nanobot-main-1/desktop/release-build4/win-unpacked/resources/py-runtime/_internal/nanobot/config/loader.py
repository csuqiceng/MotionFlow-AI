...
def get_config_path() -> Path:
    ...
    Resolution order: an explicit path set via ``set_config_path`` (e.g. the
    ``--config`` CLI flag), then the ``NANOBOT_HOME`` environment variable
    (used by self-contained builds such as the Electron desktop app to redirect
    all user data to a writable location like ``%APPDATA%``), then the
    historical ``~/.nanobot/config.json`` default.
    """

...
export interface WaitForRobotServerOptions { timeoutMs?: number; stepMs?: number; host?: string; }
  ...
  for (;;) {
    try {
      const status = await new Promise<number>((resolve, reject) => {
        const request = http.get({ host, port, path: "/health", timeout: 2000 }, (response) => { response.resume(); resolve(response.statusCode ?? 0); });
